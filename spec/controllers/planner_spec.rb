...
describe PlannerController do
  ...
  context "when user has no matching existing stock" do
    it "checking off shopping list portion and returns cupboard planner recipe portion as 100% in stock" do
      unit = create(:unit)
      ingredient = create(:ingredient)
      recipe = create(:recipe)
      portion = create(:portion, ingredient_id: ingredient.id, recipe_id: recipe.id, unit_id: unit.id)
      planner_recipe = create(
        :planner_recipe,
        recipe_id: recipe.id,
      ...
      cupboard_planner_recipes = processed_planner_recipes_with_date(user, true)
      expect(cupboard_planner_recipes.length).to eq 1
      expect(cupboard_planner_recipes.first[:plannerRecipe][:portions].length).to eq 1
      expect(cupboard_planner_recipes.first[:plannerRecipe][:portions].first[:percentInCupboards]).to eq 0

      planner_portion = PlannerShoppingListPortion.find_by(planner_recipe_id: planner_recipe.id, ingredient_id: ingredient.id)
      ...
      expect(cupboard_planner_recipes.length).to eq 1
      expect(cupboard_planner_recipes.first[:plannerRecipe][:portions].length).to eq 1
      expect(cupboard_planner_recipes.first[:plannerRecipe][:portions].first[:percentInCupboards]).to eq 100
    end
  end

  context "existing stock" do
    it "should check off shopping list portion and set cupboard planner recipe portion is 100% in stock" do
      unit = create(:unit)
      ingredient = create(:ingredient)
      recipe = create(:recipe)
      portion = create(:portion, ingredient_id: ingredient.id, recipe_id: recipe.id, unit_id: unit.id)
      stock = create(:stock, ingredient_id: ingredient.id, cupboard_id: user.cupboards.first.id, unit_id: unit.id)
      planner_recipe = create(
        :planner_recipe,
        recipe_id: recipe.id,
        planner_shopping_list_id: user.planner_shopping_list.id,
