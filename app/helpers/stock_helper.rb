module StockHelper
	def recipe_stock_matches_update(user_id)
		...
		all_cupboard_ids = CupboardUser.where(user_id: current_user[:id], accepted: true).map(&:cupboard_id)
		...
		puts "number of stock updated in the last hour: " + stock_updated_last_hour_num.to_s

		if stock_updated_last_hour_num > 0

			cupboard_stock_in_date_ingredient_ids = Stock.where(cupboard_id: active_cupboard_ids).where("use_by_date >= :date", date: Date.current - 2.days).uniq { |s| s.ingredient_id }.map{ |s| s.ingredient.id }.compact
