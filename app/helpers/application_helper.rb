module ApplicationHelper
	def icomoon(name, viewBox='')
		...
		return '<svg class="icomoon-icon icon-'.html_safe + name + '"'.html_safe + (!viewBox.empty? ? ' viewBox="'.html_safe + viewBox.to_s.html_safe + '"'.html_safe : '' ) + '><use xlink:href="'.html_safe + asset_pack_path('app/webpacker/icons/symbol-defs.svg') + '#icon-'.html_safe + name + '"></use></svg>'.html_safe + '<img class="icon-png" src="'.html_safe + asset_pack_path('PNG/app/webpacker/icons/PNG/' + image_png_name ) +'"></img>'.html_safe
	end
	def large_svg(name, viewBox='')
		return '<svg class="large-svg '.html_safe + name + '"'.html_safe + (!viewBox.empty? ? ' viewBox="'.html_safe + viewBox.to_s.html_safe + '"'.html_safe : '' )  + '><use xlink:href="'.html_safe + asset_pack_path('app/webpacker/icons/' + name + '.svg') + '#'.html_safe + name + '"></use></svg>'.html_safe
	end
	def random_name
